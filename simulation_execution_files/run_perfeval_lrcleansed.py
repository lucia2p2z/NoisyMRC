from base_simulations_perfeval import simulation_perfeval_lrcleansed
...
if __name__ == '__main__':
    ...
    summary_data, summary_results, results_models = simulation_perfeval_lrcleansed(X=X, Y=Y,
                                                                                   nvector=nvector,
                                                                                   nrep=Nrep,
                                                                                   rho1=rho1,
                                                                                   rho2=rho2)
    print('simulation perfeval LR-CLEANSED done!')
    ...
    filename1 = 'LRCLEANSED_' + dataName + '_summary_data'
    save_dataframe(
        df=summary_data,
        df_name=filename1,
        save_dir=main_dir
    )
    filename2 = 'LRCLEANSED_' + dataName + '_summary_results'
    save_dataframe(
        df=summary_results,
        df_name=filename2,
        save_dir=main_dir
    )
    filename3 = 'LRCLEANSED_' + dataName + '_models'
    save_dataframe(
        df=results_models,
        df_name=filename3,
        save_dir=main_dir
