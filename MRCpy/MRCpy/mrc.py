...
class MRC(BaseMRC):
    ...
    **phi_kwargs : Additional parameters for feature mappings.
    ...
    def compute_projection_constraints(self, pcvx):
        '''
        ...
        Parameters
        ----------
        pcvx : `array`-like of shape (`n_samples`*`n_classes`)
                cvx variable that represents a distribution  :math:`p \in \Delta(X,Y)`.

        Returns
        -------
        contraints:  `array`-like of shape (2)
                    contraints of the projection cvx problem
                    pcvx needs to be a distribution: :math:`\sum_i p_i = 1` and :math:`p_i \ge 0`.
