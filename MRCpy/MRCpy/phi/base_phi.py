...
class BasePhi():
    ...
    def est_std_noise(self, X, Y, tinvtot):
        '''
        Modification of the classic standard deviation estimate, in case fo noisy labels
        ...
        tinvtot : `array`-like of shape (`n_classes*n_dimensions`, `n_classes*n_dimensions`)
            Obtained as a kronecker product between `(T^-1)` and `I_d`
            ehere T is the transition probability matrix describing the label noise.
        ...
        -------
        lambda_ : `array`-like of shape (`n_features` * `n_classes`)
            Standard deviation of `(Ttot^-1)* phi * (Ttot^-1)^t`

        '''

        X, Y = check_X_y(X, Y, accept_sparse=True)
        cov_ = np.cov(self.eval_xy(X, Y), rowvar=False)
        lambdamatrix = tinvtot @ cov_
        lambdamatrix = lambdamatrix @ tinvtot.T

        return np.sqrt(np.diag(lambdamatrix))
